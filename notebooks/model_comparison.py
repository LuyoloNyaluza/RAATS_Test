...
def benchmark_model(model_name: str, runs: int = 3):
    if ollama is None:
        return {"model": model_name, "status": "failed", "error": "ollama not installed"}
    ollama_local = ollama  # For Pylance: guarantee non-None after the guard
    ...
    for i in range(runs):
        start = time.time()
        try:
            response = ollama_local.generate(model=model_name, prompt=prompt, options={"temperature": 0.2})
